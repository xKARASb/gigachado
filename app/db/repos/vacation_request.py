...
class VacationRequestRepository:
    ...
    def get_vacation_by_user(self, id: str) -> VacationRequestOutput:
        vacs = self.session.query(VacationRequest).filter_by(user_id=id).all()
        return [VacationRequestOutput(**vac.__dict__) for vac in vacs]
    
    def get_vacation_by_dep(self, id: str) -> VacationRequestOutput:
